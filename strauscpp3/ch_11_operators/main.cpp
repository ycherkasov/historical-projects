...
// ‘ункци€ демонстрирует работу перегруженных операторов 
// и не€вного конструктора
void show_overloads(){

	// ¬ызов не€вного конструктора
	complex_t c1 = 3;
...
// ‘ункци€ демонстрирует перегрузку опрераторов к классе,
// использующем динамическую пам€ть
void show_dynamic(){
...
void show_vector_by_val(){
	...
	size_t s = sizeof(v);
	v.push_back(1);
	// размер вектора не увеличиваетс€,
	// т.к. все элементы в динамической пам€ти
	s = sizeof(v);
	v.push_back(2);
	s = sizeof(v);
	v.push_back(3);
...
// этот пример не будет компилироватьс€ без преобразовани€ 
// в simple_ptr<music>. ƒл€ этого определен соответствующий оператор в классе
