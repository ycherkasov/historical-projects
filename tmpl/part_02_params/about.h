...
2. Нектоторые аргументы шаблонов функций невозможно получить путем вывода.
...
4. Аргументами типов не могут быть:
   ...
   - Неименованные классы и перечисления (кромеименованных через typedef)
...
3. С нетиповым параметром-указателем может фигурировать ключевое слово typename,
   если в параметре используется тип

IV SFINAE

V Шаблонные параметры шаблона
1. Шаблонный параметр определяется ключевым словом class
2. Имя шаблонного параметра шаблона не может быть использовано в шаблоне
